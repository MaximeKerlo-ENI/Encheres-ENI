...
public class EnchereDAOJdbcImpl {
	...
	public void add(Utilisateurs utilisateurs) throws SQLException {
		...
		pStmt.setVarchar(1, utilisateurs.getPseudo()); // ATTENTION, les paramètres commencent à l'index : 1
		// je remplace le deuxième ? de ma requête par la description de mon objet Avis
		pStmt.setVarchar(2, utilisateurs.getNom());
		pStmt.setvarchar(3, utilisateurs.getPrenom());
		...
		if (rs.next()) { // si jamais il y a un resultat
			utilisateurs.setId(rs.getInt(1)); //alors on utilise sa valeur pour mettre à jour l'id de l'avis
		}
	}
